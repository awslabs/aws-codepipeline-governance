...
- Option 1) Deploy Serverless Application Model function where CodePipeline and Lambda reside in different AWS Accounts
    - Replace "o-1234567890" with your AWS Organizational ID.
  ...
  bash ./scripts/sam.sh -o o-1234567890 -r Orchestration-Service
  ````

- Option 2) Deploy Serverless Application Model function where CodePipeline and Lambda reside in the same AWS Account
  ```bash
  bash ./scripts/sam.sh 
  ````
        ...
        - Name: BuildAndPackage
          Actions:
            ...
            - Name: Update-CodePipeline
              ...
              RunOrder: 2
    ```
- (*Optional*) If you would like to customize the DynamoDB Item with new parameters run the convert-2-dynamodb-item script to build the Rule to fit the DynamoDB format 
  ```bash
...
- Deploy Initial DynamoDB Item into the Scan_CodePipeline_Rules Table
  - #### Note: Run in account where the ScanCodePipeline Lambda Function solution was deployed to
  - Option 1) Run the following command if the ScanCodePipeline Lambda Function, and the CodePipeline exist in separate AWS Accounts
    ```bash
    aws dynamodb put-item --table-name Scan_CodePipeline_Rules --item file://initial-dynamodb-item-sep-acc.json
    ``` 
   
  - Option 2) Run the following command if the ScanCodePipeline Lambda Function, and the CodePipeline exist in the same AWS Account
    ```bash
